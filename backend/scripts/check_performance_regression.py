...
class PerformanceRegression:
        ...
        for test_name, current_stats in current.items():
            ...
            if baseline_ops > 0:
                ...
                ops_result = {
                    ...
                    "is_regression": ops_change_ratio
                    < -self.threshold,  # Lower ops is regression
    ...
    def generate_report(
        self, regressions: List[Dict[str, Any]], improvements: List[Dict[str, Any]]
    ) -> str:
        ...
        if regressions:
            ...
            for regression in regressions:
                ...
                report.append(
                    f"- **Change**: {regression['change_percentage']:.2f}% slower"
                )
                report.append("")
        else:
    ...
    def check_performance(
        self, current_results_file: Path, baseline_branch: str
    ) -> bool:
        """
        ...
        # Compare metrics
        regressions, improvements = self.compare_metrics(
        ...
        # Log summary
        logger.info(
            f"Performance check complete: {len(regressions)} regressions, {len(improvements)} improvements"
        )
...
def main():
    ...
    no_regressions = checker.check_performance(
        args.current_results, args.baseline_branch
    )
