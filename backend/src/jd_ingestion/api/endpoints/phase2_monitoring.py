...
async def phase2_health_check(
    ...
    try:
        ...
        health_status = {
            ...
            "metrics": {
                "active_websocket_connections": summary["websocket"][
                    "active_connections"
                ],
...
async def get_current_metrics(admin_user: AdminUser):
    """
...
async def get_performance_metrics(admin_user: AdminUser):
    """
...
async def get_websocket_metrics(admin_user: AdminUser):
    """
    ...
    return {"success": True, "data": websocket_data}


@router.get("/metrics/collaboration")
async def get_collaboration_metrics(admin_user: AdminUser):
    """
    ...
    if collaboration_data["total_operations"] > 0:
        conflict_rate = (
            collaboration_data["conflict_resolution_count"]
            / collaboration_data["total_operations"]
        )

    collaboration_data["conflict_rate"] = conflict_rate
    collaboration_data["health_status"] = (
        "healthy" if conflict_rate < 0.1 else "high_conflicts"
    )

    return {"success": True, "data": collaboration_data}


@router.get("/metrics/system")
async def get_system_metrics(admin_user: AdminUser):
    """
...
async def get_metrics_history(
    ...
    hours: int = Query(
        24, ge=1, le=168, description="Number of hours of history to retrieve"
    ),
    db: AsyncSession = Depends(get_async_session),
):
    """
    ...
    try:
        ...
        """
            ...
            {"metric_name": metric_name, "since_time": since_time},
        )
...
async def get_active_sessions(
    admin_user: AdminUser, db: AsyncSession = Depends(get_async_session)
):
    """
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"active_sessions_count": len(sessions), "sessions": sessions},
        }
...
async def record_websocket_metric(
    ...
    try:
        ...
        return {"success": True, "message": f"WebSocket {event_type} event recorded"}
...
async def record_collaboration_metric(
    admin_user: AdminUser, operation_type: str, conflict: bool = False
):
    """
...
async def get_monitoring_dashboard(admin_user: AdminUser):
    """
    ...
    try:
        ...
        if summary["collaboration"]["total_operations"] > 0:
            conflict_rate = (
                summary["collaboration"]["conflict_resolution_count"]
                / summary["collaboration"]["total_operations"]
            )
        ...
        if (
            summary["translation"]["cache_hit_ratio"] < 0.7
            and summary["translation"]["translation_requests"] > 10
        ):
            health_score -= 10
