...
const LoadingStateContext = createContext<LoadingContextType | undefined>(undefined);
...
export function LoadingProvider({ children, initialContext = 'generic' }: LoadingProviderProps) {
  const [context, setContext] = useState<LoadingContext>(initialContext);
  const [customMessage, setCustomMessage] = useState<LoadingMessage | null>(null);
  ...
  const setCustomMessageHandler = useCallback((title: string, description?: string) => {
    setCustomMessage({
      title,
      description: description || 'Please wait while we complete this operation...'
    });
  }, []);
...
export function useLoadingMessage() {
  const { setContext, setCustomMessage, clearCustomMessage, getMessage } = useLoadingContext();

  const setJobsLoading = useCallback(() => setContext('jobs'), [setContext]);
  const setSearchLoading = useCallback(() => setContext('search'), [setContext]);
  const setComparisonLoading = useCallback(() => setContext('comparison'), [setContext]);
  const setUploadLoading = useCallback(() => setContext('upload'), [setContext]);
  const setJobDetailsLoading = useCallback(() => setContext('job-details'), [setContext]);
  const setStatsLoading = useCallback(() => setContext('stats'), [setContext]);
  const setProcessingLoading = useCallback(() => setContext('processing'), [setContext]);
  const setDashboardLoading = useCallback(() => setContext('dashboard'), [setContext]);
