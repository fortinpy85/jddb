...
export function JobDetailView({
  ...
  const handleApprove = () => {
    addToast({
      title: "Job Approved",
      description: `Job ${job?.job_code} has been approved`,
      variant: "default",
    });
  };
  ...
  const handleDuplicate = () => {
    addToast({
      ...
      description: `Created a copy of job ${job?.job_code}`,
      variant: "default",
    });
  };
  ...
  const handleExport = () => {
    addToast({
      title: "Exporting Job",
      description: `Exporting job ${job?.job_code} as PDF...`,
      variant: "default",
    });
  };
  ...
  const handleArchive = () => {
    addToast({
      title: "Job Archived",
      description: `Job ${job?.job_code} has been archived`,
      variant: "default",
    });
  };
  ...
  if (loading) {
    return (
      <LoadingState
        ...
        description="Fetching job description from database"
      />
    );
  }
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {job.metadata.budget_authority !== undefined && (
                <div>
                  <label className="text-sm font-medium text-slate-600 dark:text-slate-400">
                    Budget Authority
                  </label>
                  <p className="text-sm text-slate-900 dark:text-slate-100 mt-1">
                    ${job.metadata.budget_authority.toLocaleString()}
                  </p>
                </div>
              )}
            </div>
