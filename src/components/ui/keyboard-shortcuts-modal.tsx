...
export function KeyboardShortcutsModal({ isOpen, onClose, shortcuts }: KeyboardShortcutsModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto" data-testid="keyboard-shortcuts-modal">
        <DialogHeader>
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="space-y-1">
              ...
              <ul className="text-xs text-muted-foreground space-y-1">
                <li>• Shortcuts work from anywhere except when typing in input fields</li>
                <li>• Press <Badge variant="outline" className="text-xs px-1 py-0">?</Badge> or <Badge variant="outline" className="text-xs px-1 py-0">Ctrl+H</Badge> to show this help anytime</li>
                <li>• Use <Badge variant="outline" className="text-xs px-1 py-0">/</Badge> or <Badge variant="outline" className="text-xs px-1 py-0">Ctrl+K</Badge> to quickly focus search</li>
              </ul>
            </div>
          </div>
        </div>
  ...
  return {
    ...
    KeyboardShortcutsModal: React.useCallback(
      (props: Omit<KeyboardShortcutsModalProps, 'isOpen' | 'onClose'>) => (
        <KeyboardShortcutsModal {...props} isOpen={isOpen} onClose={closeModal} />
      ),
      [isOpen, closeModal]
    )
  };
