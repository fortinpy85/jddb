...
describe("JDDBApiClient", () => {
  ...
  describe("job management methods", () => {
    test("getJobSection retrieves specific job section", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ section_type: "accountability", section_content: "Test content" }),
        text: async () => "",
      }));
    ...
    test("reprocessJob triggers job reprocessing", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ message: "Job reprocessing started", job_id: 1 }),
        text: async () => "",
      }));
  ...
  describe("preferences endpoints", () => {
    test("getAllPreferences retrieves all preferences", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ preferences: { theme: "dark" }, session_id: "123" }),
        text: async () => "",
      }));
    ...
    test("updatePreference updates a single preference", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ message: "Updated", key: "theme", value: "dark" }),
        text: async () => "",
      }));
  ...
  describe("template endpoints", () => {
    test("getTemplate retrieves a template", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ template: "Test template", classification: "EX-01" }),
        text: async () => "",
      }));
    ...
    test("generateTemplate generates a new template", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ template: "Generated template", classification: "EX-01" }),
        text: async () => "",
      }));
  ...
  describe("search utility endpoints", () => {
    test("getSearchFacets retrieves search facets", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ facets: { classifications: [], languages: [] } }),
        text: async () => "",
      }));
  ...
  describe("job creation and ingestion", () => {
    ...
    test("batchIngest processes multiple files", async () => {
      ...
      const response = await apiClient.batchIngest("/path/to/dir", { recursive: true });
  ...
  describe("additional preferences endpoints", () => {
    test("updatePreferencesBulk updates multiple preferences", async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({
        ok: true,
        json: async () => ({ message: "Updated", updated: 3, created: 1, total: 4 }),
        text: async () => "",
      }));

      const response = await apiClient.updatePreferencesBulk({ theme: "dark", language: "en" });
