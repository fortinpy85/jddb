...
export function useProgressToast() {
  ...
  const createProgressToast = useCallback(
    (options: ProgressOptions): ProgressController => {
      ...
      addToast({
        ...
        progress: {
          ...
          estimatedTimeRemaining: options.estimatedDuration
            ? formatDuration(options.estimatedDuration)
            : undefined,
        },
      });
...
export function useProgressUtils() {
  ...
  const createUploadProgress = useCallback(
    (fileName: string) => {
  ...
  const createBatchProgress = useCallback(
    (operationName: string, totalItems: number) => {
      return createProgressToast({
        title: operationName,
  ...
  const createAnalysisProgress = useCallback(
    (analysisType: string) => {
      return createProgressToast({
  ...
  // Comparison progress
  const createComparisonProgress = useCallback(
    (jobCount: number) => {
      return createProgressToast({
...
export function createStepProgress(
  steps: string[],
): (currentStep: number, customMessage?: string) => number {
